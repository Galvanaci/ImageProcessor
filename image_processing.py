...
def intensify(pixel):
    ...
    g_int = int(pixel[1])
    b_int = int(pixel[2])
    if(r_int+10 > 255 or g_int+10 > 255 or b_int+10 > 255):
        return(255, 255, 255)
    else:
        return(pixel[0]+10, pixel[1]+10, pixel[2]+10)

# detensify: pixel -> nat255 -> pixel
# darken each channel of pixel by quantity
#
# NOTE: there might be an overflow bug in this code!
# If so, reason through it and fix it. Consider the
# possible need for a function that brightens an
...
def detensify(pixel):
    ...
    g_int = int(pixel[1])
    b_int = int(pixel[2])
    if(r_int-10 < 0 or g_int-10 < 0 or b_int-10 < 0):
        return(0, 0, 0)
    else:
        return(pixel[0]-10, pixel[1]-10, pixel[2]-10)
...
# brighten: modifies image pixel array of image_surf in place
# replace each pixel with its photographic "intensify"
#
def bright(image_surf):
...
# replace each pixel with its photographic "detensify"
#
def dark(image_surf):
    ...
    for x in range(rows):
        for y in range(cols):
            pixels3d[x,y] = detensify(pixels3d[x,y])
